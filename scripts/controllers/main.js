...
app.controller('MainCtrl', function ($scope, $window, $location, $cacheFactory, SessionService, AuthenticationService, $http, $rootScope, globalAppNotification, APICallService, dropdownService) {
    ...
    $scope.$watch(function() {
        ...
        $http.post(absolutePath+'/api/group/user',{id: $scope.currentGroup}).
    ...
    $scope.getGroupList = function () {
        return $http.post(absolutePath+'/api/group/list');
    };
    ...
    $scope.setupGroup = function() {
        $scope.getGroupList().then(function (result) {
            ...
            if ($scope.currentGroup) {
                ...
                $http.post(absolutePath+'/api/survey/isPresurveyCompleted', {'groupid':$scope.currentGroup}).
                    success(function(data, status, headers, config) {
                        // this callback will be called asynchronously
                        // when the response is available
                        if (data == "true"){
    ...
    $scope.pendingTask = function(){
			    $http.post(absolutePath+'/api/mission/getPendingTask',{groupid: $scope.currentGroup}).
			    success(function(data, status, headers, config) {
			      // this callback will be called asynchronously
			      // when the response is available
