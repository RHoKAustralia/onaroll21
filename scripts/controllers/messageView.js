...
app.controller('MessageViewCtrl',function($scope,$location, $http, SessionService, $rootScope, $routeParams){
    ...
    /*$scope.$watch('notificationStatus', function (val){
	    		...
	    		$http.post(absolutePath+'/api/service/groupmessage/updateNotificationStatus', {conversationID : conversationID, status : val}).
	...
	*/
	...
	$scope.notificationSTT = function(){
			...
			$http.post(absolutePath+'/api/service/groupmessage/updateNotificationStatus', {conversationID : conversationID, st : statusVar}).
			success(function(data, status, headers, config) {
    ...
    $scope.retrieveConversation = function(){
    	...
    	if (currentRoute == "/message/view/:id"){
		    	...
		    	$http.post(absolutePath+'/api/service/groupmessage/retriveGroupMessage', {conversationID : conversationID}).
			    success(function(data, status, headers, config) {
			      //console.log(data);
    ...
    $scope.sendMessage = function(){
    	...
    	if (tempMessage){
    			$http.post(absolutePath+'/api/service/groupmessage/sendGroupMessage', {id : $routeParams['id'], message : tempMessage , userList : selectedUsers, conversationID : conversationID}).
			    success(function(data, status, headers, config) {
